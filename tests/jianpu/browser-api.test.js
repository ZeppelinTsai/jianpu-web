...
var fingeringEvent = fingeringResult.layout.lines[0].measures[0].events[0];
...
assert.ok(
  fingeringEvent.notePositions[0].octaveDotPositions.reduce(function (
    top,
    dot,
  ) {
    return Math.min(top, dot.cy - dot.r);
  }, Infinity) -
    (fingeringEvent.annotations.fingerings[0].cy +
      fingeringEvent.annotations.fingerings[0].r) >=
...
assert.match(
  fingeringResult.svg,
  /class="jianpu-fingering-text"[^>]*>1<\/text>/,
);
