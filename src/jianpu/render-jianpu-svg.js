...
function renderJianpuSvg(layout, options) {
  ...
  var notationFontFamily =
    options.notationFontFamily ||
  ...
  output.push(
    "<style>" +
      ...
      number(layout.style.numberFontSize) +
      "px;font-weight:700}" +
      ".jianpu-accidental{text-anchor:end;font-size:" +
      number(layout.style.numberFontSize * 0.7) +
      "px}" +
      ".jianpu-title{text-anchor:middle;font-size:" +
      number(layout.style.titleFontSize) +
      "px;font-weight:600;" +
      "font-family:" +
      titleFontFamily +
      "}" +
      ".jianpu-header{font-size:" +
      number(layout.style.headerFontSize) +
      "px}" +
      ...
      "font-size:" +
      number(layout.style.fingeringFontSize) +
      "px}" +
      ...
      number(layout.style.tupletFontSize || 13) +
      "px;font-style:italic;" +
      "fill:currentColor}" +
      ".jianpu-volta-label{font-size:" +
      number(layout.style.voltaFontSize || 13) +
  ...
  if (layout.header.title) {
    output.push(
      '<text class="jianpu-title" x="' +
        number(layout.header.titlePosition.x) +
        '" y="' +
        number(layout.header.titlePosition.y) +
        '">' +
        escapeXml(layout.header.title) +
        "</text>",
    );
  }
  if (layout.header.composer) {
    output.push(
      '<text class="jianpu-composer" x="' +
        number(layout.header.composerPosition.x) +
        '" y="' +
        number(layout.header.composerPosition.y) +
        '">' +
        escapeXml(layout.header.composer) +
        "</text>",
    );
  }
  ...
  if (layout.header.keyLabel) {
    output.push(
      '<text class="jianpu-header" x="' +
        number(layout.header.keyPosition.x) +
        '" y="' +
        number(layout.header.keyPosition.y) +
        '">' +
        escapeXml(layout.header.keyLabel) +
        "</text>",
    );
  }
  if (layout.header.meterPosition) {
    output.push(
      '<text class="jianpu-meter jianpu-meter-numerator" x="' +
        number(layout.header.meterPosition.x) +
        '" y="' +
        number(layout.header.meterPosition.numeratorY) +
        '">' +
        escapeXml(layout.header.meterPosition.numerator) +
        "</text>",
    );
    output.push(
      '<text class="jianpu-meter jianpu-meter-denominator" x="' +
        number(layout.header.meterPosition.x) +
        '" y="' +
        number(layout.header.meterPosition.denominatorY) +
        '">' +
        escapeXml(layout.header.meterPosition.denominator) +
        "</text>",
    );
    output.push(
      lineSvg(layout.header.meterPosition.fractionLine, "jianpu-meter-line"),
    );
  } else if (layout.header.meterLabel) {
    output.push(
      '<text class="jianpu-header" x="' +
        number(layout.header.meterTextPosition.x) +
        '" y="' +
        number(layout.header.meterTextPosition.y) +
        '">' +
        escapeXml(layout.header.meterLabel) +
        "</text>",
    );
  }
  if (layout.header.showTempo && layout.header.tempoLabel) {
    output.push(
      '<text class="jianpu-header" x="' +
        number(layout.header.tempoPosition.x) +
        '" y="' +
        number(layout.header.tempoPosition.y) +
        '">' +
        escapeXml(layout.header.tempoLabel) +
        "</text>",
    );
  }
  output.push("</g>");
  ...
  layout.lines.forEach(function (layoutLine) {
    output.push(
      '<g class="jianpu-line" data-line="' + (layoutLine.index + 1) + '">',
    );
    output.push(
      '<text class="jianpu-measure-number" x="' +
        number(layoutLine.measureNumber.x) +
        '" y="' +
        number(layoutLine.measureNumber.y) +
        '" text-anchor="' +
        escapeXml(layoutLine.measureNumber.textAnchor || "end") +
        '">' +
        escapeXml(layoutLine.measureNumber.text) +
        "</text>",
    );
    layoutLine.measures.forEach(function (measure) {
      output.push(
        '<g class="jianpu-measure" data-measure="' + (measure.index + 1) + '">',
      );
      measure.events.forEach(function (event) {
        output.push(
          '<g class="jianpu-event" data-duration-beats="' +
        ...
        event.annotations.graceNotes.forEach(function (grace) {
          if (grace.accidentalPosition) {
            output.push(
              '<text class="jianpu-accidental" x="' +
                number(grace.accidentalPosition.x) +
                '" y="' +
                number(grace.accidentalPosition.y) +
                '">' +
                escapeXml(grace.accidentalText) +
                "</text>",
            );
          }
          output.push(
            '<text class="jianpu-grace-number" x="' +
              number(grace.x) +
              '" y="' +
              number(grace.y) +
              '">' +
        ...
        event.notePositions.forEach(function (note) {
          ...
          if (clickable) {
            output.push(
              '<g class="jianpu-note" data-midi="' + note.midi + '">',
            );
          }
          if (note.accidentalPosition) {
            output.push(
              '<text class="jianpu-accidental" x="' +
                number(note.accidentalPosition.x) +
                '" y="' +
                number(note.accidentalPosition.y) +
                '">' +
                escapeXml(note.accidentalText) +
                "</text>",
            );
          }
          output.push(
            '<text class="jianpu-number" x="' +
              number(note.x) +
              '" y="' +
              number(note.y) +
              '">' +
        ...
        event.annotations.fingerings.forEach(function (annotation) {
          output.push(
            '<circle class="jianpu-fingering-circle" cx="' +
              number(annotation.cx) +
              '" cy="' +
              number(annotation.cy) +
              '" r="' +
              number(annotation.r) +
              '"/>',
          );
          output.push(
            '<text class="jianpu-fingering-text" x="' +
              number(annotation.cx) +
              '" y="' +
              number(annotation.cy) +
              '">' +
        ...
        if (event.annotations.staccato) {
          output.push(
            circleSvg(event.annotations.staccato, "jianpu-staccato-dot"),
          );
        }
        if (event.annotations.lyric) {
          var isExtender = event.annotations.lyric.text === "";
          var lyricClass =
            "jianpu-lyric" + (isExtender ? " jianpu-lyric-extend" : "");
          ...
          output.push(
            '<text class="' +
              lyricClass +
              '" x="' +
              number(event.annotations.lyric.x) +
              '" y="' +
      ...
      (measure.tupletBrackets || []).forEach(function (bracket) {
        ...
        output.push(
          '<text class="jianpu-tuplet-number" x="' +
            number(bracket.labelX) +
            '" y="' +
            number(bracket.labelY) +
            '">' +
            escapeXml(bracket.label) +
            "</text>",
        );
      });
    ...
    (layoutLine.voltaBrackets || []).forEach(function (bracket) {
      ...
      output.push(
        '<text class="jianpu-volta-label" x="' +
          number(bracket.labelX) +
          '" y="' +
          number(bracket.labelY) +
          '">' +
  ...
  (layout.pageNumbers || []).forEach(function (pageNumber) {
    output.push(
      '<text class="jianpu-page-number" x="' +
        number(pageNumber.x) +
        '" y="' +
        number(pageNumber.y) +
        '">' +
        escapeXml(pageNumber.text) +
        "</text>",
    );
  });
