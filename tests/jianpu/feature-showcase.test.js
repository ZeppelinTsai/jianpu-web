...
var outputPath = path.resolve(
  __dirname,
  "../../test-output/feature-showcase.svg",
);
...
assert.deepStrictEqual(
  tupletEvents.map(function (event) {
    return {
      ratio: event.tuplet.ratio,
      isStart: event.tuplet.isStart,
      isEnd: event.tuplet.isEnd,
    };
  }),
  [
...
assert.deepStrictEqual(
  line2.voltaBrackets.map(function (bracket) {
...
var repeatResult = jianpu.fromAbc(
  firstMeasureRepeatAbc,
  { staffWidth: 1400 },
  tuneBook.parseOnly,
);
...
var multiPageResult = jianpu.fromAbc(
  multiPageAbc,
  { staffWidth: 320, linesPerPage: 1 },
  tuneBook.parseOnly,
);
assert.ok(
...
var multiPageDisabledResult = jianpu.fromAbc(
  multiPageAbc,
  { staffWidth: 320, linesPerPage: 1, showPageNumbers: false },
  tuneBook.parseOnly,
);
assert.strictEqual(
